...
class ModelPerformanceOnline:
    ...
    def serialize_to_file(self, sql_path: str, additional_note_dataset: str='', additonal_note_model: str='') -> None:
        ...
        table_name = 'training_testing_info'
        id_col = 'sim_id'
        ...
        if searched_id == 0:
            ...
            torch.save(self.model, f'{os.path.dirname(sql_path)}/model_{searched_id}.pt')
            self.update_rule.update_logs.to_sql(f'update_logs_{searched_id}', newconn, index=True, index_label='time')
            self.update_rule.train_logs.to_sql(f'train_logs_{searched_id}', newconn, index=True, index_label='time')
            self.update_rule.test_logs.to_sql(f'test_logs_{searched_id}', newconn, index=True, index_label='time')
            self.update_rule.train_record.to_sql(f'train_record_{searched_id}', newconn, index=True, index_label='time')
            self.update_rule.train_residuals.to_sql(f'train_residuals_{searched_id}', newconn, index=True, index_label='time')
            self.update_rule.test_residuals.to_sql(f'test_residuals_{searched_id}', newconn, index=True, index_label='time')
        else:
